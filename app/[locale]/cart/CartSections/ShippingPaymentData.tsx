...
const ShippingPaymentData = () => {
 ...
 console.log(paymentData)
  ...
  useEffect(()=>{
   const fetchShippingData=async()=>{
    try{
    const data=await axios.get('http://localhost:5000/api/shipping/1')
  console.log(data,"its shipping data")
 ...
 {paymentStatus==='success' ?
  <div className="w-full flex items-center justify-center "><div onClick={handlePaymentFormChanges} className="w-24  flex items-center justify-center text-sm text-white hover:cursor-pointer bg-blue-600 hover:bg-blue-700 duration-500 p-1 rounded-xl ">Submit Changes</div></div>
  :
  paymentMethod!=='' && <div className="w-full flex items-center justify-center "><div onClick={handlePaymentForm} className="w-36  flex items-center justify-center text-sm text-white hover:cursor-pointer bg-blue-600 hover:bg-blue-700 duration-500 p-1 rounded-xl ">Submit</div></div>
 ...
 { 
  ...
  <div key={data.id} className='w-full flex items-start border border-slate-200 justify-between p-4 rounded-md bg-[#F5F5F5]'>
   {/* Left */}
   <div className='flex items-start justify-start gap-x-3 '>
    <div className='pt-1'> <input value={shippingMethod} onChange={(e)=>handleShipping(e)} id={data.id} className='border-[#D9D9D9]   size-4' name="shippingmethod" type='radio'/></div>
  <div className='flex flex-col items-start justify-start'>
